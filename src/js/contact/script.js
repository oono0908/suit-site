...
//       e.preventDefault();
//       // 最初のエラーへスクロール＆フォーカス
//       const $firstWrap = $form.find('.is-error').first();
//       const $firstInput = $firstWrap.find('input, textarea').first();
//       if($firstWrap.length) $firstWrap[0].scrollIntoView({ behavior:'smooth', block:'center' });
...
$(function () {
  ...
  function validate(){
    ...
    if(!nameVal){ showError($name, 'お名前を入力してください。'); ok = false; } else { clearError($name); }
    if(!emailVal){ showError($email, 'メールアドレスを入力してください。'); ok = false; }
    else if(!emailRe.test(emailVal)){ showError($email, 'メールアドレスの形式が正しくありません。'); ok = false; }
    else { clearError($email); }

    if(!$radios.filter(':checked').length){ showError($radios.eq(0), 'お問い合わせの種類を選択してください。'); ok = false; }
    else { clearError($radios.eq(0)); }

    if(!msgVal){ showError($msg, 'お問い合わせ内容を入力してください。'); ok = false; }
    else if(msgVal.length < 10){ showError($msg, '10文字以上で入力してください。'); ok = false; }
    else { clearError($msg); }

    return ok;
  }

  // --- サイレント判定（メッセなし）：ボタンの活性/非活性を決める ---
  function isFormValidSilently(){
    ...
    const emailRe= /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    return !!(name && emailRe.test(email) && radios.filter(':checked').length && msg.length >= 10);
  }
